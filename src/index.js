...
if (window.ethereum) {
  ...
  const texto = `<div class="animate-appear-top-bottom" style="--delay: 1" ><h1>Non-ETH Polygon MATIC browser detected. You should consider trying MetaMask!</h1></div>`;
  document.body.innerHTML = `<div class="container" id="browser">
    ${texto}
  </div>`;
}

var contractAddress = "0xBd4f668Dc74Be2c7dBaDB9d9F0471540256E1d1F";
var abi = JSON.parse(
...
web3.eth.getAccounts(function (err, accs) {
  ...
  if (accs.length == 0) {
    const texto = `<div class="animate-appear-top-bottom" style="--delay: 1"><h1>No account found!</h1></div>
      <div class="animate-appear-top-bottom" style="--delay: 2"><h2>Please, connect with the <strong>Polygon MATIC Client</strong> to start playing.</h2></div>`;
...
function deposit(){
  var val = 5000000000000000000; // 0.5
  contract.methods
    .deposit()
...
function playCaraCoroa(escolha) {
  var val = 100000000000000000; //0.1
  contract.methods
    .playCaraCoroa(escolha)
    .send({ from: account, value: val, maxFeePerGas: 50000000000,maxPriorityFeePerGas: 50000000000, gas: 400000 }) 
    .then((tx) => {
      ...
      web3.eth.getTransactionReceipt(tx.transactionHash, (err, receipt) => {
        ...
        if (
          receipt.logs[1].address == contractAddress
        )
...
function getPremioFinal() {
  contract.methods
    ...
    .then((info) => {
      let etherValue = Web3.utils.fromWei(info, "ether");
      etherValue = Number(etherValue).toFixed(3) + " MATIC";
...
function getPrice() {
  contract.methods
    ...
    .then((info) => {
      ...
      price = Number(price).toFixed(2) + " MATIC";
